...
static int ShowWeather(const char* arg)
{
	...
	cout<<"<h2> Begin Push </h2><br/>";
	pid_t id = fork();
	if(id==0){
		cout<<"<h2>Begin execl</h2><br/>"<<endl;
		execl("/home/lzh/Desktop/code/http/wwwroot/cgi/ShowWeather.py","./ShowWeather.py",NULL);
		cout<<"<h2>execl error</h2><br/>"<<endl;
		exit(1);
	}
	//if(id==waitpid(id,NULL,0))
	//{
	//	cout<<"<h2>weather already push</h2><br/>"<<endl;
	//	return 0;
	//}
